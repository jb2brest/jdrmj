...
WHERE TABLE_SCHEMA = 'dnd_characters' 
...
FROM mysql.user 
WHERE User IN (SELECT DISTINCT User FROM mysql.db WHERE Db = 'dnd_characters');
...
DELETE FROM characters WHERE user_id IN (SELECT id FROM users WHERE username = 'test_user');
...
SELECT 
    ...
    (SELECT COUNT(*) FROM information_schema.TABLES WHERE TABLE_SCHEMA = 'dnd_characters') as 'Tables',
...
SELECT 
    CASE 
        ...
        THEN '✅ Base de données correctement initialisée'
