...
echo "<h1>Initialisation des Pions de Scène</h1>";
...
try {
    ...
    ");
    echo "✅ Table scene_tokens créée/vérifiée<br>";

    // Initialiser les pions pour toutes les scènes existantes
    $stmt = $pdo->query("SELECT id FROM scenes ORDER BY id");
    $scenes = $stmt->fetchAll();

    foreach ($scenes as $scene) {
        $scene_id = $scene['id'];
        echo "<h2>Scène ID: $scene_id</h2>";
        ...
        foreach ($players as $player) {
            ...
            $stmt = $pdo->prepare("
                INSERT IGNORE INTO scene_tokens (scene_id, token_type, entity_id, position_x, position_y, is_on_map) 
                VALUES (?, 'player', ?, 0, 0, FALSE)
            ");
            $stmt->execute([$scene_id, $player['player_id']]);
        ...
        foreach ($monsters as $monster) {
            ...
            ");
            $stmt->execute([$scene_id, $monster['id']]);
