...
<?php if (!$isModal): ?>
    ...
    <script>
        ...
        window.assignObject = function(objectId, objectName) {
            ...
            document.getElementById('assignTargetType').value = '';
            document.getElementById('assignTargetId').innerHTML = '<option value="">Sélectionner d\'abord le type...</option>';
        ...
        // Fonction globale pour charger les cibles
        window.loadAssignTargets = function(type) {
            ...
            label.textContent = labels[type] || 'Cible';
            
            // Charger les cibles via AJAX
            fetch(`api/get_assign_targets.php?type=${type}`)
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const assignTargetId = document.getElementById('assignTargetId');
            
            if (assignTargetType) {
                assignTargetType.addEventListener('change', function() {
                    const targetType = this.value;
                    assignTargetId.innerHTML = '<option value="">Chargement...</option>';
                    assignTargetId.disabled = true;
                    
                    if (targetType) {
                        loadAssignTargets(targetType);
                    }
                });
            }
            
            // Charger les cibles selon le type (fonction globale)
            window.loadAssignTargets = function(type) {
                const select = document.getElementById('assignTargetId');
                const label = document.getElementById('assignTargetLabel');
