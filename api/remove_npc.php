...
try {
    ...
    if ($success) {
        echo json_encode([
            'success' => true,
            'message' => 'PNJ supprimé avec succès'
        ]);
    } else {
        throw new Exception('Erreur lors de la suppression du PNJ');
    }
    
} catch (Exception $e) {
    error_log("Erreur remove_npc.php: " . $e->getMessage());
    echo json_encode([
        'success' => false,
        'error' => $e->getMessage()
    ]);
