...
// Debug logs
error_log("DEBUG view_character.php: Session ID = " . session_id());
error_log("DEBUG view_character.php: User ID = " . ($_SESSION['user_id'] ?? 'NULL'));
error_log("DEBUG view_character.php: Username = " . ($_SESSION['username'] ?? 'NULL'));
error_log("DEBUG view_character.php: User Role = " . ($_SESSION['role'] ?? 'NULL'));
error_log("DEBUG view_character.php: Is DM = " . (isDM() ? 'true' : 'false'));
error_log("DEBUG view_character.php: Is Admin = " . (User::isAdmin() ? 'true' : 'false'));
error_log("DEBUG view_character.php: Is DM or Admin = " . (User::isDMOrAdmin() ? 'true' : 'false'));
error_log("DEBUG view_character.php: Session data = " . print_r($_SESSION, true));

if (!isset($_GET['id'])) {
    error_log("DEBUG view_character.php: No ID parameter, redirecting to characters.php");
...
error_log("DEBUG view_character.php: Request parameters - character_id=$character_id, dm_campaign_id=" . ($dm_campaign_id ?? 'NULL') . ", created=" . ($character_created ? 'true' : 'false'));
error_log("DEBUG view_character.php: Full GET parameters = " . print_r($_GET, true));

// Récupération et validation du personnage
$characterObject = Character::findById($character_id);

if (!$characterObject) {
    error_log("DEBUG view_character.php: Character ID $character_id not found, redirecting to characters.php");
    header('Location: characters.php');
    exit();
}
...
if (!$isOwner && !$isDM && !$isAdmin) {
    error_log("DEBUG view_character.php: Access denied - neither owner, DM, nor admin, redirecting to characters.php");
    header('Location: characters.php');
    exit();
}
...
// Contrôle d'accès: propriétaire OU MJ de la campagne liée OU Admin
$canView = ($character['user_id'] == $_SESSION['user_id']);

// Les admins peuvent voir tous les personnages
if (!$canView && User::isAdmin()) {
    $canView = true;
    error_log("DEBUG view_character.php: Admin access granted for all characters");
}

error_log("DEBUG view_character.php: Second permission check - canView = " . ($canView ? 'true' : 'false'));
error_log("DEBUG view_character.php: Character user_id = " . $character['user_id']);
error_log("DEBUG view_character.php: Session user_id = " . $_SESSION['user_id']);
error_log("DEBUG view_character.php: User::isDMOrAdmin() = " . (User::isDMOrAdmin() ? 'true' : 'false'));
error_log("DEBUG view_character.php: User::isAdmin() = " . (User::isAdmin() ? 'true' : 'false'));
error_log("DEBUG view_character.php: dm_campaign_id = " . ($dm_campaign_id ?? 'NULL'));

if (!$canView && User::isDMOrAdmin() && $dm_campaign_id) {
    error_log("DEBUG view_character.php: Entering advanced permission check");
    ...
    error_log("DEBUG view_character.php: Campaign found = " . ($campaign ? 'true' : 'false'));
    error_log("DEBUG view_character.php: User::isAdmin() = " . (User::isAdmin() ? 'true' : 'false'));
    
    if (User::isAdmin()) {
        $ownsCampaign = true; // Les admins peuvent voir toutes les feuilles
        error_log("DEBUG view_character.php: Admin access granted");
    } else if ($campaign) {
        $ownsCampaign = $campaign->canModify($_SESSION['user_id'], User::getRole());
        error_log("DEBUG view_character.php: Campaign ownership check = " . ($ownsCampaign ? 'true' : 'false'));
    }

    if ($ownsCampaign) {
        error_log("DEBUG view_character.php: User owns campaign, checking member status");
        ...
        error_log("DEBUG view_character.php: Owner is member = " . ($isMember ? 'true' : 'false'));
        error_log("DEBUG view_character.php: Owner has applied = " . ($hasApplied ? 'true' : 'false'));
        $canView = ($isMember || $hasApplied);
        error_log("DEBUG view_character.php: Final canView = " . ($canView ? 'true' : 'false'));
    } else {
        error_log("DEBUG view_character.php: User does not own campaign");
    }
} else {
    error_log("DEBUG view_character.php: Skipping advanced permission check - canView=" . ($canView ? 'true' : 'false') . ", isDMOrAdmin=" . (User::isDMOrAdmin() ? 'true' : 'false') . ", dm_campaign_id=" . ($dm_campaign_id ?? 'NULL'));
}

if (!$canView) {
    error_log("DEBUG view_character.php: Final access denied - redirecting to characters.php");
    header('Location: characters.php');
    exit();
}
