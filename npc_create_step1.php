...
$page_title = "Création de PNJ - Étape 1";
...
if (!$session_id) {
    $session_id = createNPCCreationSession($user_id);
    if (!$session_id) {
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['action']) && $_POST['action'] === 'select_class') {
    ...
    error_log("DEBUG npc_create_step1.php - POST action: " . ($_POST['action'] ?? 'NOT_SET'));
    error_log("DEBUG npc_create_step1.php - POST class_id: " . ($class_id ?? 'NOT_SET'));
    
    if ($class_id > 0) {
        // Sauvegarder le choix de classe
        $saveResult = saveNPCCreationStep($user_id, $session_id, 2, ['class_id' => $class_id]);
        error_log("DEBUG npc_create_step1.php - Save result: " . ($saveResult ? 'SUCCESS' : 'FAILED'));
        
        if ($saveResult) {
            error_log("DEBUG npc_create_step1.php - Redirecting to step2");
...
error_log("DEBUG npc_create_step1.php - Session ID: " . $session_id);
error_log("DEBUG npc_create_step1.php - User ID: " . $user_id);
error_log("DEBUG npc_create_step1.php - Session Data: " . print_r($sessionData, true));
error_log("DEBUG npc_create_step1.php - Selected Class ID: " . ($selectedClassId ?? 'NULL'));
?>
...
<body>
        ...
        <div class="container">
            <div class="row align-items-center">
                <div class="col-md-8">
                    ...
                    <p class="mb-0">Étape 1 sur 12 - Choisissez la classe</p>
                </div>
                <div class="col-md-4">
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        <h3><i class="fas fa-shield-alt me-2"></i>Choisissez la classe du PNJ</h3>
                        <p class="mb-0 text-muted">La classe détermine les capacités principales et le style de jeu du PNJ.</p>
                    </div>
                    <div class="card-body">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Si une classe est déjà sélectionnée, activer le bouton
            if (selectedClassIdInput.value) {
                continueBtn.disabled = false;
            }
        });
    </script>
